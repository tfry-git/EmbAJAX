...
// statics
AjaneOutputDriverBase *AjaneBase::_driver;
char AjaneBase::itoa_buf[ITOA_BUFLEN];

////////////////////////////// AjaneOutputDriverBase ////////////////////

void AjaneOutputDriverBase::printFiltered(const char* value, bool quoted, bool HTMLescaped) {
...
//////////////////////// AjaneConnectionIndicator ///////////////////////

void AjaneConnectionIndicator::print() const {
    ...
    _driver->printQuoted(_content_ok ? _content_ok : "<span class=\"AjaneStatusOK\" style=\"background-color:green;\">OK</span>");
    _driver->printContent (";}},\n"
                           "'out': function() {if (this.misses < 5) { if(++(this.misses) >= 5) this.div.innerHTML=");
    _driver->printQuoted(_content_fail ? _content_fail : "<span class=\"AjaneStatusOK\" style=\"background-color:red;\">FAIL</span>");
    _driver->printContent(";}}\n"
                          "}\n</script></div>");
}
...
AjaneElement::AjaneElement(const char* id) : AjaneBase() {
    _id = id;
    _flags = 1 << AjaneBase::Visibility | 1 << AjaneBase::Enabledness;
    revision = 1;
}

bool AjaneElement::sendUpdates(uint16_t since, bool first) {
...
void AjaneElement::setBasicProperty(uint8_t num, bool status) {
...
void AjaneElement::setChanged() {
    revision = _driver->setChanged();
}

bool AjaneElement::changed(uint16_t since) {
...
void AjaneElement::printTextInput(uint SIZE, const char* _value) const {
...
void AjaneBase::printChildren(AjaneBase** _children, uint NUM) const {
    for (uint i = 0; i < NUM; ++i) {
        _children[i]->print();
    }
}

bool AjaneBase::sendUpdates(AjaneBase** _children, uint NUM, uint16_t since, bool first) {
    for (uint i = 0; i < NUM; ++i) {
...
AjaneElement* AjaneBase::findChild(AjaneBase** _children, uint NUM, const char*id) const {
    for (uint i = 0; i < NUM; ++i) {
        AjaneElement* child = _children[i]->toElement();
        if (child) {
...
void AjaneMutableSpan::print() const {
...
const char* AjaneMutableSpan::value(uint8_t which) const {
    if (which == AjaneBase::Value) return _value;
    return AjaneElement::value(which);
}

bool AjaneMutableSpan::valueNeedsEscaping(uint8_t which) const {
    if (which == AjaneBase::Value) return !basicProperty(AjaneBase::HTMLAllowed);
    return AjaneElement::valueNeedsEscaping(which);
}

const char* AjaneMutableSpan::valueProperty(uint8_t which) const {
    if (which == AjaneBase::Value) return "innerHTML";
    return AjaneElement::valueProperty(which);
}

void AjaneMutableSpan::setValue(const char* value, bool allowHTML) {
    ...
    _value = value;
    setBasicProperty(AjaneBase::HTMLAllowed, allowHTML);
    setChanged();
}

//////////////////////// AjaneSlider /////////////////////////////

AjaneSlider::AjaneSlider(const char* id, int16_t min, int16_t max, int16_t initial) : AjaneElement(id) {
    _value = initial;
    _min = min;
    _max = max;
}

void AjaneSlider::print() const {
    _driver->printContent("<input id=");
    _driver->printQuoted(_id);
...
const char* AjaneSlider::value(uint8_t which) const {
    if (which == AjaneBase::Value) return itoa(_value, itoa_buf, 10);
    return AjaneElement::value(which);
}

const char* AjaneSlider::valueProperty(uint8_t which) const {
    if (which == AjaneBase::Value) return "value";
    return AjaneElement::valueProperty(which);
}

void AjaneSlider::updateFromDriverArg(const char* argname) {
    char buf[16];
    _driver->getArg(argname, buf, 16);
    _value = atoi(buf);
}

void AjaneSlider::setValue(int16_t value) {
    _value = value;
    setChanged();
}
...
AjaneColorPicker::AjaneColorPicker(const char* id, uint8_t r, uint8_t g, uint8_t b) : AjaneElement(id) {
    _r = r;
    _g = g;
    _b = b;
}

void AjaneColorPicker::print() const {
    _driver->printContent("<input id=");
    _driver->printQuoted(_id);
    _driver->printContent(" type=\"color\" value=");
    _driver->printQuoted(value());
    // see AjaneTextInput::print(): Use onInput(), instead of onChange().
    _driver->printContent(" onInput=\"clearTimeout(this.debouncer); this.debouncer=setTimeout(function() {doRequest(this.id, this.value);}.bind(this),1000);\"/>");
}
...
const char* AjaneColorPicker::value(uint8_t which) const {
    if (which != AjaneBase::Value) return AjaneElement::value(which);
...
const char* AjaneColorPicker::valueProperty(uint8_t which) const {
    if (which == AjaneBase::Value) return "value";
    return AjaneElement::value(which);
}

void AjaneColorPicker::setColor(uint8_t r, uint8_t g, uint8_t b) {
    _r = r;
    _g = g;
    _b = b;
    setChanged();
...
uint8_t AjaneColorPicker::red() const {
    return _r;
}

uint8_t AjaneColorPicker::green() const {
    return _g;
}

uint8_t AjaneColorPicker::blue() const {
    return _b;
}
...
void AjaneColorPicker::updateFromDriverArg(const char* argname)  {
    char buf[9];
...
AjanePushButton::AjanePushButton(const char* id, const char* label, void (*callback)(AjanePushButton*)) : AjaneElement (id) {
    _label = label;
    _callback = callback;
    setBasicProperty(AjaneBase::HTMLAllowed, true);
}

void AjanePushButton::print() const {
...
void AjanePushButton::setText(const char* label, bool allowHTML) {
    _label = label;
    setBasicProperty(AjaneBase::HTMLAllowed, allowHTML);
    setChanged();
}

const char* AjanePushButton::value(uint8_t which) const {
    if (which == AjaneBase::Value) return _label;
    return AjaneElement::value(which);
}

bool AjanePushButton::valueNeedsEscaping(uint8_t which) const {
    if (which == AjaneBase::Value) return !basicProperty(AjaneBase::HTMLAllowed);
    return AjaneElement::valueNeedsEscaping(which);
}

const char* AjanePushButton::valueProperty(uint8_t which) const {
    if (which == AjaneBase::Value) return "innerHTML";
    return AjaneElement::valueProperty(which);
}

void AjanePushButton::updateFromDriverArg(const char* argname) {
    _callback(this);
}

//////////////////////// AjaneCheckButton /////////////////////////////

AjaneCheckButton::AjaneCheckButton(const char* id, const char* label, bool checked) : AjaneElement(id) {
    _label = label;
    _checked = checked;
    radiogroup = 0;
}

void AjaneCheckButton::print() const {
...
const char* AjaneCheckButton::value(uint8_t which) const {
    if (which == AjaneBase::Value) return _checked ? "true" : "";
    return AjaneElement::value(which);
}

void AjaneCheckButton::updateFromDriverArg(const char* argname) {
    char buf[16];
    _driver->getArg(argname, buf, 16);
    _checked = (buf[0] == 't');
...
const char* AjaneCheckButton::valueProperty(uint8_t which) const {
    if (which == AjaneBase::Visibility) return "parentNode.style.display";
    if (which == AjaneBase::Value) return "checked";
    return AjaneElement::valueProperty(which);
}

void AjaneCheckButton::setChecked(bool checked) {
...
//////////////////////// AjaneOptionSelect(Base) ///////////////

void AjaneOptionSelectBase::print(const char* const* _labels, uint8_t NUM) const {
...
void AjaneOptionSelectBase::selectOption(uint8_t num) {
    _current_option = num;
    setChanged();
}

uint8_t AjaneOptionSelectBase::selectedOption() const {
    return _current_option;
}

const char* AjaneOptionSelectBase::value(uint8_t which) const {
    if (which == AjaneBase::Value) return (itoa(_current_option, itoa_buf, 10));
    return AjaneElement::value(which);
}

const char* AjaneOptionSelectBase::valueProperty(uint8_t which) const {
    if (which == AjaneBase::Value) return ("value");
    return AjaneElement::valueProperty(which);
}

void AjaneOptionSelectBase::updateFromDriverArg(const char* argname) {
    _current_option = atoi(_driver->getArg(argname, itoa_buf, ITOA_BUFLEN));
}

//////////////////////// AjanePage /////////////////////////////

void AjaneBase::printPage(AjaneBase** _children, uint NUM, const char* _title, const char* _header_add) const {
...
void AjaneBase::handleRequest(AjaneBase** _children, uint NUM, void (*change_callback)()) {
    ...
    AjaneElement *element = (id[0] == '\0') ? 0 : findChild(id);
    if (element) {
